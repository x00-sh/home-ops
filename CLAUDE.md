...
**Put new apps in a new namespace.** Every `kubernetes/apps/<ns>/kustomization.yaml`
that the template owns (`kube-system`, `network`, `default`, `cert-manager`,
`flux-system`) is regenerated by `just configure`, so adding an app to one of
them means editing a file that will be silently reverted. A namespace the
template knows nothing about — `storage` is the first — keeps a hand-written
`kustomization.yaml` that survives. Copy `kubernetes/apps/storage/` for the
shape; it must keep `components: - ../../components/sops`, or `postBuild`
substitution has no `cluster-secrets` to read and every app in the namespace
fails.

Adding files under `talos/all/` works the same way: topf globs the directory,
and a filename the template does not generate is left alone.
